...
/** Cache ISR — emprego não muda intra-day. 6h é folgado. */
...
export type PnadData = {
  ...
  composicao: {
    serie: PnadComposicaoPonto[];
    categorias: string[];
  };
  setor: {
    serie: PnadSetorPonto[];
    categorias: string[];
  };
  metadata: {
    fonte: string;
...
// ---------------------------------------------------------------------------
// CAGED — saldo total nacional (IPEADATA)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// CAGED — quebras (microdados agregados)
// ---------------------------------------------------------------------------
...
export type CagedQuebrasData = {
  ...
  metadata: {
    fonte: string;
    nota: string;
    cnae_para_setor: Record<string, string>;
  };
};
...
export async function loadCagedFull(): Promise<{
  ...
  const [total, quebras] = await Promise.all([loadCagedTotal(), loadCagedQuebras()]);
  return { total, quebras };
}

// Categorias e ordens canônicas (espelham build_emprego_caged_quebras.py)
...
/** Agrupa as 12 faixas oficiais em 5 grupos legíveis. */
