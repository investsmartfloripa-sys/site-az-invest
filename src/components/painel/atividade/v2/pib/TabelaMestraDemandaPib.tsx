...
/**
 * Tabela mestra da demanda — a planilha de referência do trimestre mais recente
 * pela ótica da DESPESA: uma linha por componente da demanda (6, ordem da SCN),
 * uma coluna por medida. NÍVEL (índice de volume SA e R$ reais SA), VARIAÇÃO
 * (YoY, acum. 4T, acum. ano, QoQ SA, com sinal e cor) e PESO (% do PIB nominal).
 * Tudo do último ponto de cada série já presente no JSON — sem cálculo derivado,
 * é o "raio-X" tabular que acompanha os gráficos. Exporta em CSV (padrão pt-BR).
 *
 * Leitura: importações entram como vazamento (alta de importações subtrai do
 * PIB; nas contribuições o sinal é trocado). A variação de estoques fecha a
 * identidade C + G + FBCF + ΔEstoques + (X − M). `valores_reais_sa` e
 * `estrutura_nominal` são opcionais no tipo — colunas mostram "—" quando faltam.
 */

// 6 recortes da demanda em ordem fixa da SCN.
const COMPONENTES: { key: string }[] = [
...
export function TabelaMestraDemandaPib({
  pib,
  geradoEm,
}: {
  ...
  // Último ponto (mais recente) de cada série; chaves por recorte abaixo.
  ...
  const linhas = useMemo<LinhaDemanda[]>(
    () =>
      COMPONENTES.map((c) => ({
        key: c.key,
        rotulo: labels[c.key] ?? LABELS_PIB_FALLBACK[c.key] ?? c.key,
        indiceSa: num(indiceUlt, `sa_${c.key}`),
        reaisSa: num(reaisUlt, c.key),
        yoy: num(variacaoUlt, `yoy_${c.key}`),
        acum4t: num(variacaoUlt, `acum_4t_${c.key}`),
        acumAno: num(variacaoUlt, `acum_ano_${c.key}`),
        qoqSa: num(variacaoUlt, `qoq_sa_${c.key}`),
        pesoPib: num(estruturaUlt, `${c.key}_pct_pib`),
      })),
  ...
  const baixar = () => {
    baixarCsv(
      `pib-tabela-mestra-demanda-${trimRef}.csv`,
      [
        "componente",
  ...
  return (
    <ChartCard
      title={`Tabela mestra da demanda — ${fmtTrimCurto(trimRef)}`}
      subtitle="Os 6 componentes da demanda (ótica da despesa, ordem da SCN) × nível, variação e peso, no trimestre mais recente. Nas taxas, verde = expansão e vermelho = queda. Importações entram como vazamento (alta subtrai do PIB)."
      toolbar={
        <button
          type="button"
          onClick={baixar}
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[860px] border-collapse text-xs tabular-nums">
          ...
          <tbody>
            {linhas.map((l) => (
              <tr key={l.key} className="border-b border-zinc-100 text-zinc-700">
                <td className="py-1.5 pr-3 font-medium text-[#132960]">{l.rotulo}</td>
