"""Build do bloco ComÃ©rcio Exterior por Produto (Comex Stat / SECEX-MDIC).

Gera `data-pipeline/out/contas_externas_comex.json` e upload pra
`data/contas_externas_comex.json`.

Coleta:
- Top NCM exportaÃ§Ãµes (12m)
- Top NCM importaÃ§Ãµes (12m)
- ExportaÃ§Ãµes por seÃ§Ã£o do SH (24m mensais)
- ImportaÃ§Ãµes por seÃ§Ã£o do SH (24m mensais)
- Top destinos das exportaÃ§Ãµes (12m)
- Top origens das importaÃ§Ãµes (12m)
...
- filterSizeLimit=12000 (necessÃ¡rio pra carregar dataset completo de NCM)
...
# Categorias amigÃ¡veis para agregar NCMs em produtos populares
# Cada lista sÃ£o prefixos NCM (4 ou 6 dÃ­gitos) que pertencem ao grupo
PRODUTO_CATEGORIAS_EXPORT: list[tuple[str, list[str]]] = [
    ("Soja e derivados", ["1201", "2304", "1507"]),
    ("PetrÃ³leo e derivados", ["2709", "2710", "2711"]),
    ("MinÃ©rio de ferro", ["2601"]),
    ("Carne bovina", ["0201", "0202"]),
    ("Carne de frango", ["0207"]),
    ("CafÃ©", ["0901"]),
    ("AÃ§Ãºcar", ["1701", "1702"]),
    ("Celulose", ["4703"]),
    ("AlgodÃ£o", ["5201", "5202", "5203"]),
    ("Milho", ["1005"]),
    ("Ouro", ["7108"]),
    ("Ferro e aÃ§o (semimanufaturados)", ["7207", "7208", "7209"]),
    ("VeÃ­culos", ["8703", "8704", "8708", "8429"]),
    ("AviÃµes", ["8802"]),
]
PRODUTO_CATEGORIAS_IMPORT: list[tuple[str, list[str]]] = [
    ("Adubos e fertilizantes", ["3102", "3103", "3104", "3105"]),
    ("PetrÃ³leo e derivados", ["2709", "2710", "2711"]),
    ("EletrÃ´nicos (chips/celulares)", ["8542", "8517"]),
    ("VeÃ­culos e peÃ§as", ["8703", "8704", "8708"]),
    ("Medicamentos", ["3004"]),
    ("MÃ¡quinas industriais", ["8429", "8443", "8479"]),
    ("Pesticidas e defensivos", ["3808"]),
    ("PlÃ¡sticos", ["3901", "3902", "3904"]),
    ("Trigo", ["1001"]),
    ("Cobre (catodos)", ["7403"]),
]
...
def _br_yyyymm_pair(months_back_from: int, months_back_to: int = 0) -> tuple[str, str]:
    """Retorna (from, to) em formato YYYY-MM relativos ao mÃªs corrente.
    A API SECEX tem defasagem ~1 mÃªs â€” usamos mÃªs corrente -1 como ponta."""
    hoje = datetime.now(timezone.utc).date()
    # Para o mÃªs anterior
...
def fetch_top_ncm(flow: str, period_from: str, period_to: str) -> list[dict]:
    """Top NCMs no perÃ­odo acumulado (sem monthDetail)."""
    payload = {
        "flow": flow,
        "monthDetail": False,
...
def fetch_section_3m(flow: str, period_from: str, period_to: str) -> list[dict]:
    """ExportaÃ§Ãµes ou importaÃ§Ãµes por seÃ§Ã£o do SH, mensal (max 3m por chamada)."""
    payload = {
        "flow": flow,
        "monthDetail": True,
        "period": {"from": period_from, "to": period_to},
...
def aggregate_ncm_by_category(ncm_rows: list[dict], categorias: list[tuple[str, list[str]]]) -> list[dict]:
    """Soma valores de NCMs por categoria amigÃ¡vel."""
    out: list[dict] = []
    used = set()
...
def section_24m_by_chunks(flow: str, n_chunks: int = 8) -> dict[str, dict[str, float]]:
    ...
    for i in range(n_chunks):
        # 24-3*(i+1)+1 atÃ© 24-3*i â€” ou seja, mais antigo no inÃ­cio
        ...
        print(f"  [{flow}] section chunk {i+1}/{n_chunks}: {pf} â†’ {pt}")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build Comex Stat por produto/paÃ­s/seÃ§Ã£o")
    ...
    # NCM Ã— 12m estoura limite da API (retorna 0 rows mesmo com success=true).
    # Pra ranking de produtos/paÃ­ses, usamos o trimestre mais recente disponÃ­vel
    # (3 meses fechados, mÃªs corrente -1 como ponta). Ã‰ lente o bastante pra
    # atualizar mensalmente e ainda forte o suficiente como ranking.
    pf_12m, pt_12m = _br_yyyymm_pair(3, 1)  # ex.: fev/26 â†’ abr/26
    print(f"Janela 3m (ranking): {pf_12m} â†’ {pt_12m}")
    ...
    # ---- 5. ExportaÃ§Ãµes por seÃ§Ã£o 24m mensal ----
    print("== Section exports 24m (em chunks) ==")
    section_exp = section_24m_by_chunks("export", n_chunks=4)
    # Identifica top 6 seÃ§Ãµes no total agregado pra simplificar visual
    ...
    # Constroi sÃ©rie temporal sÃ³ com top 6 + Outros
    ...
    # ---- 6. ImportaÃ§Ãµes por seÃ§Ã£o 24m mensal ----
    ...
    payload = {
        ...
        "fonte_principal": "Comex Stat (SECEX / MDIC) â€” POST /general",
        "periodo_12m": {"from": pf_12m, "to": pt_12m},
        # Top NCMs (com nome amigÃ¡vel)
        "top_ncm_export_12m": [
            {
                "ncm": r.get("coNcm"),
                "nome": r.get("ncm"),
        ...
        # SÃ©ries por seÃ§Ã£o SH (US$ bi mensal)
        ...
        "metadata": {
            "fonte": "SECEX/MDIC â€” Comex Stat",
            "endpoint": "POST /general (api-comexstat.mdic.gov.br)",
            "nota": "Categorias de produto agregadas por prefixo NCM. Top NCMs limitado a 15. SÃ©ries mensais por seÃ§Ã£o SH em US$ bi.",
        },
    }

    out_file.write_text(json.dumps(payload, ensure_ascii=False))
    size_kb = out_file.stat().st_size / 1024
    print(f"\nâœ“ Gerado {out_file} ({size_kb:.1f} KB)")
    ...
    print(f"  SeÃ§Ãµes export 24m: {len(serie_exp_secoes)} meses, top6: {top6_exp_sec[:3]}...")
