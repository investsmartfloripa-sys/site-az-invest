...
- **Versões neutras em `core/` (21/09/2026, cockpit do PIB):** `Divisor` (label caixa alta + filete + ?), `CockpitChip`/`tomPorSinal` (o número dinâmico mora aqui, nunca no título), `SectionTabs` (pill bar de abas irmãs configurável por `{base, abas}` — `AtividadeTabs` é a instância da área Atividade), `DifusaoBar` (barra empilhada de contagem sobe/estável/cai), `MiniSpark` (sparkline SVG com linha do zero). Preset `10y` no `AzPeriodSelector` para séries trimestrais (40 trimestres). `DataStamp` imprime trimestre como `2T26` (passe o trim cru, nunca `trimIsoCentral`).
- **Área Atividade é cockpit** (decisão do dono, 21/09/2026: "essa área foi construída como uma história, com os valores nos títulos — quero um cockpit"). Página PIB = `atividade/v2/pib/cockpit/PibCockpit.tsx` (abas → header técnico com difusão → 4 KPIs + fila secundária → 7 divisores + grade `xl:grid-cols-2` → ficha técnica), 14 cards, cobertura 100% da CNT via tabelas mestras + lupa de 9 lentes (`cockpit-shared.ts`). Base única de rebase da página: média de 2019 = 100. PIM/PMC/PMS e o hub ainda estão no template narrativo — próxima leva.
- **Contas Externas é cockpit** (decisão do dono, 23/09/2026: "cockpit da nossa balança de pagamentos"). Página = `contas-externas/cockpit/ContasExternasCockpit.tsx` (abas `ContasExternasTabs` Balanço de pagamentos | Câmbio → header técnico → 4 KPIs + fila de 6 → 8 divisores + grade `xl:grid-cols-2` → ficha técnica), tabela mestra com 100% do BPM6 mensal, PII, fluxo cambial, Focus, câmbio real/PPC/fundamentos e pauta. `StackedBpChart` é o gráfico de decomposição por sinal. Atenção: o `mesIso` de Atividade SEMPRE anexa "-01" — em payloads com `mes` "YYYY-MM-01" use `isoMes` de `contas-externas/cockpit/cockpit-shared.ts`.
